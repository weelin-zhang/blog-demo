...
class ArchDateView(ListView):
    ...
    def get_queryset(self):
        ...
        return super(ArchDateView, self).get_queryset().filter(
            created_time__year=year,
            created_time__month=month,
        )

    # def get_context_data(self, **kwargs):
    #     context = super(ArchDateView, self).get_context_data(**kwargs)
    #     print(context)
    #     return context


class ArchCategoryView(ListView):
    model = Post
    template_name = 'index.html'
    context_object_name = 'posts'
    ...
    def get_queryset(self):
        ...
        return super(ArchCategoryView, self).get_queryset().filter(category=cate)


class ArchTagView(ListView):
    model = Post
    template_name = 'index.html'
    context_object_name = 'posts'

    def get_queryset(self):  # 重写get_queryset方法,返回我们需要的数据
        tag = get_object_or_404(Tag, name=self.kwargs.get('name'))

        return super(ArchTagView, self).get_queryset().filter(tags=tag)
...
class testView(View):
    def post(self, request):
        t_g = request.GET.get('t_g', 1)
        t_p = request.POST.get('t_p', 2)
        print(t_g, t_p)
